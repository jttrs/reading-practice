...
import type { DecodingData } from '../decoding/types'
...
export default function SightWordsModule() {
  ...
  const practiceItems = useMemo(
    () => sightWords.map((w) => ({ item: w, elementId: `sw:${w.word}` })),
    [sightWords],
  )

  const session = usePracticeSession<SightWord>('sight-words', practiceItems)
  const tts = useTTS('sight-words')
